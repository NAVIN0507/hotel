...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  if (!room.data) return { title: "Room Not Found" };

  return {
    title: room.data.name,
    description: `Stay in our ${room.data.name} at Brundhavan Garden Resort. ${room.data.description || "Luxury accommodation in Thandigudi."}`,
    openGraph: {
      images: room.data.image ? [room.data.image] : [],
    },
  };
}
